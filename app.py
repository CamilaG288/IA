...
# Usar o estoque já reservado como base
estoque_atual_df = estoque_reservado_df.copy()
...
# Calcular reservas de componentes com base nos pedidos abertos
...
reservas_componentes = reservas_componentes.groupby('COMPONENTE')['Reserva Total'].sum().reset_index()

# Criar cópia do estoque para fazer reservas
estoque_reservado_df = estoque_df.copy()
estoque_reservado_df['Estoque Disponivel'] = estoque_reservado_df['QUANTIDADE']

# Reservar estoque de componentes para atender pedidos em carteira
for _, row in reservas_componentes.iterrows():
    componente = row['COMPONENTE']
    reserva_total = row['Reserva Total']
    if componente in estoque_reservado_df['COMPONENTE'].values:
        estoque_reservado_df.loc[estoque_reservado_df['COMPONENTE'] == componente, 'Estoque Disponivel'] -= reserva_total

# Agora usar estoque_reservado_df para montagem
for _, row in reservas_componentes.iterrows():
    componente = row['COMPONENTE']
    reserva_total = row['Reserva Total']
    if componente in estoque_atual_df['COMPONENTE'].values:
        estoque_atual_df.loc[estoque_atual_df['COMPONENTE'] == componente, 'Estoque Disponivel'] -= reserva_total

# Criar dicionário de reservas por produto
# Lista para armazenar produtos não atendíveis
produtos_nao_atendiveis = set()

# (Removido loop de reserva pedido por pedido; reserva já foi feita globalmente)

# (Removido clip lower=0 para permitir valores negativos e validar corretamente o estoque)
...
# Criar uma cópia do estoque reservado para montagem
estoque_montagem_df = estoque_atual_df.copy()

for _, row in curva_ordenada.iterrows():
    if row['PRODUTO'] in produtos_nao_atendiveis:
        continue
    produto = row['PRODUTO']
    ...
    # Utilizar unidades possíveis diretamente
    unidades_possiveis = unidades_possiveis_iniciais
...
# Ajustar índice para começar em 1
pedidos_possiveis.index = pedidos_possiveis.index + 1

# Exibir tabela
st.dataframe(pedidos_possiveis.reset_index().applymap(format_number))
