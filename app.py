...
# Reservar estoque globalmente por componente antes de calcular montagem possível
...
# Reservar componentes iterando por pedidos
for _, pedido in pedidos_abertos_df.iterrows():
    produto = pedido['PRODUTO']
    quantidade_produzir = pedido['QUANTIDADE PRODUZIR']

    estrutura_pedido = estrutura_pedidos_abertos_df[estrutura_pedidos_abertos_df['PRODUTO'] == produto]

    componente_faltando = False

    for comp in estrutura_pedido.itertuples():
        componente = comp.COMPONENTE
        quantidade_necessaria = comp.QUANTIDADE * quantidade_produzir

        estoque_atual = estoque_atual_df.loc[estoque_atual_df['COMPONENTE'] == componente, 'Estoque Disponivel'].values

        if estoque_atual.size > 0 and estoque_atual[0] >= quantidade_necessaria:
            if not componente_faltando:
                estoque_atual_df.loc[estoque_atual_df['COMPONENTE'] == componente, 'Estoque Disponivel'] -= quantidade_necessaria
        else:
            produtos_nao_atendiveis.add(produto)
            componente_faltando = True
            break
